...
const MAX_MARK_TIMING_US = 32767;
const MAX_SPACE_TIMING_US = 50000;
...
function buildTimingLevels(maxTimingUs) {
  const ratio = maxTimingUs / MIN_TIMING_US;
  const levels = [];
...
const MARK_TIMING_LEVELS = buildTimingLevels(MAX_MARK_TIMING_US);
const SPACE_TIMING_LEVELS = buildTimingLevels(MAX_SPACE_TIMING_US);
// Compatibility aliases for callers that only need the original mark grid.
const TIMING_LEVELS = MARK_TIMING_LEVELS;
const MAX_TIMING_US = MAX_MARK_TIMING_US;

function buildWords() {
  const words = [];
  for (const mark of MARK_TIMING_LEVELS) {
    for (const space of SPACE_TIMING_LEVELS) {
...
function findTimingLevelIn(levels, value) {
...
function findTimingLevel(value) {
  return findTimingLevelIn(TIMING_LEVELS, value);
}

function encodePair(mark, space) {
  const markMatch = findTimingLevelIn(MARK_TIMING_LEVELS, mark);
  const spaceMatch = findTimingLevelIn(SPACE_TIMING_LEVELS, space);
  return {
