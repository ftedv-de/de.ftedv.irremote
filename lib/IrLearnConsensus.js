...
class IrLearnConsensus {
  ...
  hasCompatiblePulseShape(left, right) {
    if (!this.sameStructure(left, right)) return false;

    const a = left.raw.code;
    const b = right.raw.code;
    const pairCount = Math.floor(a.length / 2);
    if (pairCount < 3) return false;

    // Leader timings must match. Payload spaces are allowed to switch between
    // the binary short/long classes, but marks must keep the same protocol shape.
    if (IrLearnConsensus.relativeError(a[0], b[0]) > this.timingTolerance) return false;
    if (IrLearnConsensus.relativeError(a[1], b[1]) > this.timingTolerance) return false;

    for (let pair = 1; pair < pairCount; pair += 1) {
      if (IrLearnConsensus.relativeError(a[pair * 2], b[pair * 2]) > this.timingTolerance) {
        return false;
      }
    }
  ...
  findConsensus(captures, {
    allowVariablePayload = Array.isArray(captures) && captures.length >= this.maxCaptures,
  } = {}) {
    ...
    // The fallback is deliberately weaker than the normal timing consensus and
    // is only enabled by the caller once the learning run has exhausted its attempts.
    if (!allowVariablePayload) return null;
  ...
  findVariablePayloadConsensus(captures) {
    const groups = [];
    for (const candidate of captures) {
      let group = groups.find((entries) => this.hasCompatiblePulseShape(entries[0], candidate));
      if (!group) {
        group = [];
        groups.push(group);
      }
    ...
    for (const group of groups) {
      ...
      if (!best || result.support > best.support || (
        result.support === best.support && result.totalBitDistance < best.totalBitDistance
      )) {
        best = result;
      }
    }
  ...
  selectVariablePayloadMedoid(group) {
    ...
    for (const candidate of group) {
      ...
      const totalBitDistance = close.reduce((sum, distance) => sum + distance, 0);
      const maxBitDistance = close.length ? Math.max(...close) : 0;
      const averageBitDistance = totalBitDistance / Math.max(1, support - 1);

      if (!best || support > best.support || (
        support === best.support && totalBitDistance < best.totalBitDistance
      )) {
        best = {
          capture: candidate,
          clusterSize: support,
          ...
          maxBitDistance,
          averageBitDistance,
          // Kept as an alias for compatibility with callers/tests written before
          // the metric was named explicitly.
          bitDistance: totalBitDistance,
          score: averageBitDistance,
  ...
  payloadBitDistance(left, right) {
    if (!this.hasCompatiblePulseShape(left, right)) return Number.POSITIVE_INFINITY;
    const a = left.raw.code;
    const b = right.raw.code;
    const pairCount = Math.floor(a.length / 2);
    if (pairCount < 3) return Number.POSITIVE_INFINITY;

    // The first pair is the leader and the final pair is trailer/idle. Everything
    // in between is payload. With a 68-entry NEC-style frame this correctly
    // compares all 32 payload bits.
    const aSpaces = [];
    const bSpaces = [];
    ...
    const aProfile = IrLearnConsensus.twoLevelProfile(aSpaces);
    const bProfile = IrLearnConsensus.twoLevelProfile(bSpaces);
    if (!aProfile || !bProfile) return Number.POSITIVE_INFINITY;

    if (IrLearnConsensus.relativeError(aProfile.shortMedian, bProfile.shortMedian) > this.timingTolerance) {
      return Number.POSITIVE_INFINITY;
    }
    if (IrLearnConsensus.relativeError(aProfile.longMedian, bProfile.longMedian) > this.timingTolerance) {
      return Number.POSITIVE_INFINITY;
    }

    let differingBits = 0;
    for (let index = 0; index < aSpaces.length; index += 1) {
      const aBit = aSpaces[index] > aProfile.threshold;
      const bBit = bSpaces[index] > bProfile.threshold;
  ...
  static twoLevelProfile(values) {
    const threshold = IrLearnConsensus.twoLevelThreshold(values);
    if (threshold === null) return null;

    const shortValues = values.filter((value) => value <= threshold);
    const longValues = values.filter((value) => value > threshold);
    if (!shortValues.length || !longValues.length) return null;

    return {
      threshold,
      shortMedian: IrLearnConsensus.median(shortValues),
      longMedian: IrLearnConsensus.median(longValues),
    };
  }
