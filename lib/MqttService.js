...
module.exports = class MqttService {
  ...
  async learn(timeout = 30000) {
    ...
    try {
      ...
      this.app.log(`MQTT learn request published: ${requestId}`);
    } catch (error) {
  ...
  onMessage(topic, payload) {
    ...
    try {
      ...
      if (!pending) {
        this.app.log(`MQTT IR response ignored: no pending request ${message.requestId || '<missing>'}`);
        return;
      }
      ...
      this.app.log(`MQTT IR response received: requestId=${message.requestId}, format=${code.format}`);
      if (code.format === 'pronto') {
        this.app.log(`MQTT learned ProntoHex: ${code.code}`);
      } else {
        this.app.log(`MQTT learned raw IR: carrier=${code.carrier}Hz, code=${code.code.join(',')}`);
      }
