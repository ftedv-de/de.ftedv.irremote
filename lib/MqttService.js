...
module.exports = class MqttService {
  ...
  validateCode(value) {
    if (!value || !['raw', 'pronto'].includes(value.format)) {
      throw new Error('IR code format must be raw or pronto');
    }

    if (value.format === 'raw') {
      if (!Array.isArray(value.code) || value.code.length === 0
        || value.code.length % 2 !== 0
        || value.code.some((item) => !Number.isInteger(item) || item === 0)) {
      ...
      if (!Number.isInteger(carrier) || carrier < 30000 || carrier > 45000) {
        throw new Error('Raw IR carrier must be between 30000 and 45000 Hz');
      }
      return { format: 'raw', code: [...value.code], carrier };
    }

    if (typeof value.code !== 'string' || !value.code.trim()) {
      throw new Error('ProntoHex IR code must be a string');
    }

    const words = value.code.trim().split(/\s+/);
    if (words.length < 6 || words.some((word) => !/^[0-9a-fA-F]{4}$/.test(word))) {
      throw new Error('ProntoHex IR code must contain 4-digit hexadecimal words');
    }

    const firstSequencePairs = Number.parseInt(words[2], 16);
    const repeatSequencePairs = Number.parseInt(words[3], 16);
    const expectedWords = 4 + ((firstSequencePairs + repeatSequencePairs) * 2);
    if (words.length !== expectedWords) {
      throw new Error(`ProntoHex length mismatch: expected ${expectedWords} words, got ${words.length}`);
    }
    if (Number.parseInt(words[1], 16) === 0) {
      throw new Error('ProntoHex carrier frequency word must not be zero');
    }

    return { format: 'pronto', code: words.map((word) => word.toUpperCase()).join(' ') };
  }
