...
module.exports = class MqttService {
  ...
  async connect(config) {
    ...
    this.client.on('error', (error) => this.app.error('MQTT error', error));
    this.client.on('message', (topic, payload) => this.onMessage(topic, payload));

    await new Promise((resolve, reject) => {
      const timer = this.app.homey.setTimeout(() => reject(new Error('MQTT connection timed out')), 10000);
      this.client.once('connect', () => {
        this.app.homey.clearTimeout(timer);
        this.client.subscribe(config.topics.received, (error) => {
  ...
  validateCode(value) {
    ...
    if (value.format === 'raw') {
      if (!Array.isArray(value.code) || value.code.length === 0
        || value.code.some((item) => !Number.isInteger(item))) {
        throw new Error('Raw IR code must be a non-empty integer array');
      }
    ...
    return { format: 'pronto', code: value.code.trim() };
  }
