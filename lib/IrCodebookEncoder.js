...
class IrCodebookEncoder {
  ...
  encodeTimings(timings, section) {
    ...
    for (let offset = 0; offset < timings.length; offset += 2) {
      ...
      if (mark > MAX_MARK_TIMING_US) {
        throw new Error(`IR mark ${mark} us exceeds Homey's ${MAX_MARK_TIMING_US} us codebook limit`);
      }
      if (space > MAX_SPACE_TIMING_US) {
        if (offset + 2 !== timings.length) {
          throw new Error(
            `IR space ${space} us exceeds the ${MAX_SPACE_TIMING_US} us codebook limit inside the frame`,
          );
        }
        space = MAX_SPACE_TIMING_US;
